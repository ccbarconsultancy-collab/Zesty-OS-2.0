...
class AudioPlugin(Plugin):
    ...
    def _on_encoded_audio(self, encoded_data: bytes) -> None:
        ...
        Battery optimization: 当处于 ARMED (IDLE) 状态时，麻克风音频仅用于
        本地唤醒词检测，无需 Opus 编码或发送到服务器。跳过编码回调以避免
        不必要的 CPU 和电池消耗。

        编码仍会在 ARMED 时发生（在 audio_codec.py 的 _input_callback 中），
        但这里在 IDLE 时直接返回，避免 schedule_command_nowait 的开销。
        """
        try:
            if not self._cmd:
                return
            # ARMED (IDLE) 状态下跳过服务器音频发送
